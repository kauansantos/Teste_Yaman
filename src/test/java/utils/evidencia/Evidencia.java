...
public class Evidencia {
	...
	static audiUtil util = new audiUtil();

	public Evidencia(String cenario, String testName) {
		document = new Document();
		passeds = 0;
		errors = 0;
		nomeCt =cenario +" - "+ testName;
	...
	public static void print(String text, WebElement... values) {
		addNewPage();
		if (values.length > 0) {
			audiUtil.centralize(values);
			CssIterator.markWebElement(values);
		}
		insertPrint();
		if (values.length > 0) {
			CssIterator.markOffWebElement(WebDriverFactory.getCurrentRunningDriver(), values);
		}
		addText(text);
	}
